...
#define ASF_APPID @"ExHdKzwAUfqQexuec8epvd4GjXzpG9xQoUYGVkqYQAff"
#define ASF_SDKKEY @"4JDK1fswMDxj6ENVRTbw3gWrWXLBsHCRySvxjeDMeERN"
#define DETECT_MODE ASF_DETECT_MODE_VIDEO
#define ASF_FACE_NUM 6
#define ASF_FACE_SCALE 16
#define ASF_FACE_COMBINEDMASK                                                                      \
ASF_FACE_DETECT | ASF_FACERECOGNITION | ASF_AGE | ASF_GENDER | ASF_FACE3DANGLE
...
- (void)uninitProcessor {
    if (_processSemaphore &&
        0 == dispatch_semaphore_wait(_processSemaphore, DISPATCH_TIME_FOREVER)) {
        dispatch_semaphore_signal(_processSemaphore);
        _processSemaphore = NULL;
    }
    
    if (_processFRSemaphore &&
        0 == dispatch_semaphore_wait(_processFRSemaphore, DISPATCH_TIME_FOREVER)) {
...
- (void)setDetectFaceUseFD:(BOOL)detectFaceUseFD {
    if (_detectFaceUseFD == detectFaceUseFD) return;
    ...
    if (0 == dispatch_semaphore_wait(_processSemaphore, 0)) {
        ...
        do {
            ...
            if (ASF_MOK != [self.arcsoftFace getFace3DAngle:&face3DAngle] ||
                face3DAngle.num != multiFaceInfo.faceNum)
                break;
            
            ASF_AgeInfo ageInfo = {0};
            if (ASF_MOK != [self.arcsoftFace getAge:&ageInfo] ||
                ageInfo.num != multiFaceInfo.faceNum)
                break;
            
            ASF_GenderInfo genderInfo = {0};
            if (ASF_MOK != [self.arcsoftFace getGender:&genderInfo] ||
                genderInfo.num != multiFaceInfo.faceNum)
                break;
        ...
        if (0 == dispatch_semaphore_wait(_processFRSemaphore, 0)) {
            __block ASF_CAMERA_INPUT_DATA offscreenProcess =
            [self copyCameraDataForProcessFR:cameraData];
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^() {
                
                if (!weakSelf.frModelVersionChecked) { weakSelf.frModelVersionChecked = YES; }
                
                if (detectFace) {
                    ASF_FaceFeature faceFeature = {0};
                    NSTimeInterval begin        = [[NSDate date] timeIntervalSince1970];
                    MRESULT mr                  = [self.arcsoftFace
                                                   extractFaceFeatureWithWidth:offscreenProcess->i32Width
                    ...
                    if (mr == ASF_MOK) {
                        ...
                        for (ASFRPerson *person in persons) {
                            ...
                            MRESULT mr =
                            [self.arcsoftFace compareFaceWithFeature:&faceFeature
                        ...
                        if (maxScore > scoreThreshold) { currentPerson.name = recognizedName; }
                ...
                dispatch_sync(dispatch_get_main_queue(), ^{
                    if (self.delegate &&
                        [self.delegate respondsToSelector:@selector(processRecognized:)])
...
- (BOOL)registerDetectedPerson:(NSString *)personName {
    ...
    if (registerPerson == nil || registerPerson.registered) return NO;
...
- (ASF_CAMERA_INPUT_DATA)copyCameraDataForProcessFR:(ASF_CAMERA_INPUT_DATA)pOffscreenIn {
    if (pOffscreenIn == MNull) { return MNull; }
    ...
    if (ASVL_PAF_NV12 == pOffscreenIn->u32PixelArrayFormat) {
        memcpy(_cameraDataForProcessFR->ppu8Plane[0], pOffscreenIn->ppu8Plane[0],
               pOffscreenIn->i32Height * pOffscreenIn->pi32Pitch[0]);
        
        memcpy(_cameraDataForProcessFR->ppu8Plane[1], pOffscreenIn->ppu8Plane[1],
