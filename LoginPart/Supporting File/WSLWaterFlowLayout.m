...
-(NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    NSMutableArray *result = [self.attrsArray mutableCopy];
    ...
    QMUILogInfo(@"wate flow layout",@"y = %f,y' = %f",contentOffset.y,self.collectionViewContentSize.height + 150 - headerSize.height);
  
    CGRect frame = CGRectMake(0, MAX(0, MIN(self.collectionViewContentSize.height - DEVICE_HEIGHT + NavigationContentTop + TabBarHeight, contentOffset.y)), headerSize.width, headerSize.height);  //offset y by the amount scrolled
...
//      QMUILogInfo(@"water flow layout",@"height = %f, width = %f",size.height,size.width);
    }
